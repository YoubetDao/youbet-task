...
export const PulsatingButton = React.forwardRef<HTMLButtonElement, PulsatingButtonProps>(
  ({ className, children, pulseColor = '#0096ff', duration = '1.5s', ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
      ...
      >
        ...
        <div className="size-full absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-lg bg-inherit" />
      </button>
    )
  },
)
